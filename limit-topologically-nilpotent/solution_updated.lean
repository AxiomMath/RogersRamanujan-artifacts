...
theorem hasBasis_nhds_zero_addSubgroup_mul_le {R : Type*} [CommRing R] [TopologicalSpace R]
    [StrongNonarchimedeanRing R] :
    (nhds (0 : R)).HasBasis (fun M : AddSubgroup R ↦ IsOpen (ofClass% M) ∧ M * M ≤ M) (↑) :=
  hasBasis_nhds_zero_openSubrng.to_hasBasis'
    (fun M _ ↦ ⟨M.toAddSubgroup, ⟨M.isOpen, AddSubgroup.mul_le_iff.mpr fun _ hx _ hy ↦
      M.mul_mem hx hy⟩, le_rfl⟩)
    fun M hM ↦ M.mem_nhds_zero hM.1

theorem exists_addSubgroup_mul_le_sub {R : Type*} [CommRing R] [TopologicalSpace R]
    [StrongNonarchimedeanRing R] {M₀ : AddSubgroup R} (hM₀ : IsOpen (ofClass% M₀)) (l : R) :
    ∃ M : AddSubgroup R, IsOpen (ofClass% M) ∧ M * M ≤ M ∧ M ≤ M₀ ∧ ∀ x ∈ M, l * x ∈ M₀ := by
  obtain ⟨M, hM, hlM⟩ := hasBasis_nhds_zero_addSubgroup_mul_le.mem_iff.mp <|
    Filter.inter_mem (M₀.mem_nhds_zero hM₀) <|
      ((continuous_const_mul l).continuousAt <| by simpa using M₀.mem_nhds_zero hM₀)
  exact ⟨M, hM.1, hM.2, fun x hx ↦ (hlM hx).1, fun x hx ↦ (hlM hx).2⟩
